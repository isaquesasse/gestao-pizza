...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Uma única regra alimenta consulta rápida, produção, início e venda rápida:
  // estoque físico menos todos os pedidos ativos da semana escolhida.
  ...
  const DEFAULT_LOJA_CONFIG = {
    id: true,
    loja_nome: "Sasse's Pizza",
    loja_subtitulo: "Pizza congelada artesanal",
    loja_aberta: true,
    mensagem_fechada: "Estamos fechados agora. Volte mais tarde.",
    horario_atendimento: "",
    pedido_minimo: 0,
    aceita_retirada: true,
    aceita_entrega: true,
    aceita_disponiveis: true,
    aceita_encomenda: true,
    taxa_por_km: 1,
    entrega_gratis_km: 3,
    frete_minimo: 0,
    cidades_atendidas: [],
    ...
    texto_whatsapp_pedido: "Quero receber os avisos desse pedido por aqui.",
  };

  const normalizeConfigArray = (value) => {
  ...
  const getLojaConfig = () => ({ ...DEFAULT_LOJA_CONFIG, ...(database.loja_config || {}) });
  ...
  const getPedidoBaseValue = (pedido) => {
    const subtotal = safeNumber(pedido?.subtotal_produtos, NaN);
    if (Number.isFinite(subtotal) && subtotal > 0) return subtotal;
    const total = safeNumber(pedido?.valorTotal, NaN);
    if (Number.isFinite(total) && total > 0) return total;
    return (pedido?.items || []).reduce((acc, item) => acc + safeNumber(item.preco) * Number(item.qtd || 0), 0);
  };

  const getPedidoFinalValue = (pedido) => {
    const final = safeNumber(pedido?.valorFinal, NaN);
    if (Number.isFinite(final) && final > 0) return final;
    return getPedidoBaseValue(pedido);
  };

  const getPedidoDiscountInfo = (pedido) => {
    const base = getPedidoBaseValue(pedido);
    const final = getPedidoFinalValue(pedido);
    const amount = Math.max(0, base - final);
    const percent = base > 0 && amount > 0.009 ? Math.round((amount / base) * 100) : 0;
    return { base, final, amount, percent };
  };

  const renderPedidoDiscountBadge = (pedido) => {
    const info = getPedidoDiscountInfo(pedido);
    if (!info.percent) return "";
    return `<span class="discount-badge">-${info.percent}%</span>`;
  };
  ...
  const getAvailableStockForPizza = (pizzaId, items = [], releaseMap = new Map(), weekStart = getWeekStart()) => {
    const pizza = database.estoque.find((p) => p.id === pizzaId);
    if (!pizza) return 0;
    const alreadyInCart = getStockMapFromItems(items).get(pizzaId) || 0;
    const availableForWeek = getPizzaSobraForWeek(pizzaId, weekStart);
  ...
  const getSystemAlerts = () => {
    const sortByEntrega = (a, b) => `${a.dataEntrega || ""} ${a.cliente || ""}`.localeCompare(`${b.dataEntrega || ""} ${b.cliente || ""}`);
    // "Pendente" é o rótulo interno de "Aguardando confirmação". Pedidos de
    // semanas futuras ficam pendentes de propósito (updatePedidoStatus recusa
    // confirmá-los para não baixar o estoque atual), então avisar sobre eles
    // seria só ruído: só contam os que já dá para confirmar hoje.
    const aguardandoConfirmacao = database.pedidos
      .filter((p) => (p.status || "Pendente") === "Pendente" && !isFutureWeek(getPedidoDataEntrega(p)))
      .sort(sortByEntrega);
    const prontosParaConcluir = database.pedidos.filter((p) => p.status === "Pronto").sort(sortByEntrega);
    return {
      aguardandoConfirmacao,
      prontosParaConcluir,
      confirmar: aguardandoConfirmacao.length,
      concluir: prontosParaConcluir.length,
      total: aguardandoConfirmacao.length + prontosParaConcluir.length
    };
  };

  // Selo numérico na aba Pedidos (barra de cima e barra de baixo do celular),
  // para o aviso aparecer mesmo com outra aba aberta.
  const refreshPedidosTabBadge = (count) => {
    document.querySelectorAll('.tab-link[data-tab="pedidos"]').forEach((link) => {
      ...
      badge.title = `${count} pedido(s) aguardando confirmação`;
  ...
  const refreshNotificationBadge = () => {
    ...
    refreshPedidosTabBadge(alerts.confirmar);
    ...
    const partes = [];
    if (alerts.confirmar) partes.push(`${alerts.confirmar} aguardando confirmação`);
    if (alerts.concluir) partes.push(`${alerts.concluir} pronto(s) para concluir`);
    btn.title = partes.join(" · ") || "Sem pendências";
  };

  const abrirPedidosAguardandoConfirmacao = () => {
    const select = document.getElementById("filter-modal-status");
    if (select) select.value = "Pendente";
    document.querySelectorAll("#filter-status-chips button").forEach((b) => b.classList.toggle("active", b.dataset.status === "Pendente"));
    if (isMobileViewport()) openDesktopTabOnMobile("pedidos");
    else openTab("pedidos");
    renderPedidos();
  };

  // Aviso único por sessão ao abrir a gestão. Não manda e-mail de propósito:
  // é só um lembrete na tela de quem já entrou no painel.
  const PENDING_ALERT_SESSION_KEY = "sassesPendingOrdersAlertShown";
  const hidePendingOrdersAlert = () => document.getElementById("pending-orders-alert")?.setAttribute("hidden", "");
  const maybeAnnouncePendingOrders = () => {
    const box = document.getElementById("pending-orders-alert");
    if (!box || sessionStorage.getItem(PENDING_ALERT_SESSION_KEY) === "true") return;
    const { confirmar, aguardandoConfirmacao } = getSystemAlerts();
    if (!confirmar) return;
    sessionStorage.setItem(PENDING_ALERT_SESSION_KEY, "true");
    const title = document.getElementById("pending-alert-title");
    const copy = document.getElementById("pending-alert-copy");
    if (title) title.textContent = confirmar === 1 ? "1 pedido aguardando confirmação" : `${confirmar} pedidos aguardando confirmação`;
    if (copy) {
      const nomes = aguardandoConfirmacao.slice(0, 2).map((p) => p.cliente || "Cliente").join(", ");
      copy.textContent = confirmar > 2 ? `${nomes} e mais ${confirmar - 2}. Confirme para entrarem na produção.` : `${nomes}. Confirme para entrar na produção.`;
    }
  ...
  const setCurrentSeller = (id) => {
    if (id) {
      ...
      if (!isSellerAdmin(seller)) sessionStorage.removeItem(ADMIN_SESSION_KEY);
    } else {
  ...
  window.openNotificationsModal = () => {
    ...
    let contentHTML = "";
    if (total === 0) {
      contentHTML = `<p class="empty-state">Tudo em dia: nenhum pedido aguardando confirmação ou conclusão.</p>`;
    } else {
      if (confirmar) {
  ...
  const isFutureWeek = (dateValue) => getWeekStart(dateValue) > getWeekStart();
...
Deseja adicionar esse frete ao Valor Final?`)) {
  ...
  const syncClientsFromOrders = async () => {
    ...
    if (clientsToUpsert.length > 0) {
      const { data, error } = await supabaseClient
        ...
        .select();
      if (error) console.warn("Não foi possível sincronizar clientes dos pedidos:", error.message);
  ...
  const renderLojaConfig = () => {
    ...
    setInputValue('cfg-loja-aberta', cfg.loja_aberta !== false);
    setInputValue('cfg-aceita-retirada', cfg.aceita_retirada !== false);
    setInputValue('cfg-aceita-entrega', cfg.aceita_entrega !== false);
    setInputValue('cfg-aceita-disponiveis', cfg.aceita_disponiveis !== false);
    setInputValue('cfg-aceita-encomenda', cfg.aceita_encomenda !== false);
    setInputValue('cfg-pedido-minimo', Number(cfg.pedido_minimo || 0));
    setInputValue('cfg-horario', cfg.horario_atendimento || '');
    setInputValue('cfg-mensagem-fechada', cfg.mensagem_fechada || '');
    ...
    setInputValue('cfg-cidades-bloqueadas', normalizeConfigArray(cfg.cidades_bloqueadas));
    setInputValue('cfg-whatsapp', cfg.whatsapp_number || '');
    setInputValue('cfg-pix-ativo', cfg.pix_ativo !== false);
    setInputValue('cfg-dinheiro-ativo', cfg.dinheiro_ativo !== false);
    setInputValue('cfg-cartao-ativo', cfg.cartao_ativo !== false);
    setInputValue('cfg-combinar-ativo', cfg.combinar_ativo !== false);
    ...
    setInputValue('cfg-texto-whatsapp', cfg.texto_whatsapp_pedido || '');
  };

  const getLojaConfigFormPayload = () => ({
    id: true,
    loja_nome: "Sasse's Pizza",
    loja_subtitulo: "Pizza congelada artesanal",
    loja_aberta: getCheckboxValue('cfg-loja-aberta'),
    aceita_retirada: getCheckboxValue('cfg-aceita-retirada'),
    aceita_entrega: getCheckboxValue('cfg-aceita-entrega'),
    aceita_disponiveis: getCheckboxValue('cfg-aceita-disponiveis'),
    aceita_encomenda: getCheckboxValue('cfg-aceita-encomenda'),
    pedido_minimo: getNumberValue(getInputValue('cfg-pedido-minimo'), 0),
    horario_atendimento: getInputValue('cfg-horario').trim(),
    mensagem_fechada: getInputValue('cfg-mensagem-fechada').trim(),
    ...
    cidades_bloqueadas: normalizeConfigArray(getInputValue('cfg-cidades-bloqueadas')),
    whatsapp_number: getInputValue('cfg-whatsapp').replace(/\D/g, ''),
    pix_ativo: getCheckboxValue('cfg-pix-ativo'),
    dinheiro_ativo: getCheckboxValue('cfg-dinheiro-ativo'),
    cartao_ativo: getCheckboxValue('cfg-cartao-ativo'),
    combinar_ativo: getCheckboxValue('cfg-combinar-ativo'),
    ...
    texto_whatsapp_pedido: getInputValue('cfg-texto-whatsapp').trim(),
  ...
  const openModal = (modalId, title, contentHTML, callback) => {
  ...
  window.openHistoryModal = (clientId) => {
    ...
    const summaryHTML = insight ? `
      ...
      <p class="small-muted"><b>Resumo do histórico:</b> ${insight.reasons.join("; ")}.</p>
    ` : "";
      ...
      pedidosCliente.forEach((p) => {
        ...
        tableHTML += `
                    <tr>
                        ...
                        <td><span class="status-${normalizeStatusClass(p.status)}">${escapeHTML(p.status || "Pendente")}</span>${payTag}</td>
                    </tr>
                `;
      });
  ...
  const renderPedidos = () => {
    ...
    filteredData.forEach((p) => {
      ...
      row.innerHTML = `
                ...
                <td data-label="Valores"><b>${formatCurrency(valorExibido)}</b>${renderPedidoDiscountBadge(p)}<br><small class="admin-only">Produtos: ${formatCurrency(descontoInfo.base)}</small>${descontoInfo.amount > 0 ? `<br><small class="discount-detail">Desconto: ${formatCurrency(descontoInfo.amount)}</small>` : ""}<br>${pagamentoTag}</td>
  ...
  const renderActionButtons = (pedido) => {
    ...
    const confirmarBtn = isFutureWeek(getPedidoDataEntrega(pedido))
      ? `<button class="action-btn confirm-btn" type="button" disabled title="Fica pendente até a semana de entrega para preservar o estoque atual">Agendado</button>`
      : `<button class="action-btn confirm-btn" onclick="window.updatePedidoStatus('${pedido.id}', 'Confirmado')">Confirmar</button>`;
  ...
  window.updatePedidoStatus = async (id, newStatus) => {
    ...
    if (pedido && targetStatus === "Confirmado" && isFutureWeek(getPedidoDataEntrega(pedido))) {
  ...
  document.getElementById("btn-add-item-pedido")?.addEventListener("click", () => {
    ...
    if (pizzaId === "outro") {
      ...
      const disponivel = getAvailableStockForPizza(pizzaId, pedidoAtualItems, new Map(), weekStart);
      if (qtd > disponivel) {
...
Deseja lançar mesmo assim como encomenda/produção pendente?`);
  ...
  const registrarNovoPedido = async () => {
    ...
    const pedidoVendedorInput = document.getElementById("pedido-vendedor");
    const vendedor = (pedidoVendedorInput?.value || getCurrentSeller()?.nome || "").trim();
    if (pedidoVendedorInput && !pedidoVendedorInput.value && vendedor) pedidoVendedorInput.value = vendedor;
    const cidade = document.getElementById("pedido-cidade").value;
    const endereco = document.getElementById("pedido-endereco")?.value || "";
    ...
    const newPedidoData = {
      ...
      observacoes,
      status: getInitialPedidoStatus(dataEntrega),
      tipo_pedido: getInitialPedidoType(dataEntrega),
    ...
    const pedidoMessage = isFutureWeek(dataEntrega)
      ? "Pedido agendado para uma semana futura, sem baixar o estoque da semana atual."
      : pedidoSalvoRecord?.estoque_baixado
        ? "Pedido registrado e estoque reservado."
        : "Pedido registrado e confirmado para produção.";
  ...
  const renderProductionDemand = () => {
    ...
    const weekStats = getPizzaWeekStats(selectedWeek);

    let productionData = weekStats
  ...
  const renderIngredientes = () => {
    ...
    filteredData.forEach((item) => {
      ...
      row.innerHTML = `<td data-label="Nome">${escapeHTML(item.nome || "")}</td><td data-label="Qtd. em Estoque">${(item.qtd || 0).toFixed(3)}</td><td data-label="Estoque Mínimo">${(item.estoqueMinimo || 0).toFixed(3)}</td><td data-label="Custo (p/ Unidade)" class="admin-only">${formatCurrency(item.custo)}</td><td data-label="Ações"><button class="action-btn edit-btn" onclick="window.editIngrediente('${escapeAttr(item.id)}')">Editar</button><button class="action-btn remove-btn" onclick="window.removeIngrediente('${escapeAttr(item.id)}')">Remover</button></td>`;
    });
  ...
  const renderReceitaIngredientesList = () => {
    ...
    if (receitaAtualIngredientes) {
      receitaAtualIngredientes.forEach((item, index) => {
        ...
        container.innerHTML += `<div class="receita-ingrediente-item"><p><span>${(item.qtd || 0).toFixed(3)} x</span> ${escapeHTML(ingrediente ? ingrediente.nome : "Ingrediente removido")}</p><button type="button" class="btn-remove-item" onclick="window.removeIngredienteDaReceita(${index})">X</button></div>`;
      });
    }
  };
  ...
  const renderReceitas = () => {
    ...
    filteredData.forEach((receita) => {
      ...
      const row = tbody.insertRow();
      row.innerHTML = `<td data-label="Pizza">${escapeHTML(pizza.nome)} (${escapeHTML(pizza.tamanho || "")})</td><td data-label="Ingredientes"><small>${escapeHTML(ingredientesList)}</small></td><td data-label="Custo Total" class="admin-only">${formatCurrency(custoTotal)}</td><td data-label="Ações"><button class="action-btn edit-btn" onclick="window.editReceita('${escapeAttr(receita.pizzaId)}')">Editar</button><button class="action-btn remove-btn" onclick="window.removeReceita('${escapeAttr(receita.pizzaId)}')">Remover</button></td>`;
    });
  };
  ...
  window.openEditPedidoModal = (id) => {
    ...
    const descontoAtual = getPedidoDiscountInfo(pedido);
    const valorAtual = descontoAtual.final;
    const descontoAtualTexto = descontoAtual.percent ? `${descontoAtual.percent}%` : "";
    ...
    const formHTML = `
      <form id="edit-pedido-form" class="edit-form edit-order-form">
        ...
        <div class="edit-section">
          ...
          <div class="edit-grid three">
            ...
            <label>Desconto rápido
              <input type="text" id="edit-pedido-desconto" inputmode="decimal" placeholder="0%" value="${escapeAttr(descontoAtualTexto)}">
            </label>
          </div>
          <label class="toggle-line">
        ...
        <div class="edit-section">
          <div class="edit-section-title"><span>Valores e observações</span><small>O total final já considera desconto, quando houver</small></div>
          <div class="edit-grid two">
            ...
            <div class="edit-discount-summary" id="edit-pedido-desconto-resumo"></div>
          </div>
          <label>Observações
  ...
  const updateTotalEditPedido = () => {
    ...
    const resumo = document.getElementById("edit-pedido-desconto-resumo");
    if (resumo) {
      const economia = Math.max(0, totalBruto - total);
      resumo.innerHTML = desconto > 0 ? `<span class="discount-badge">-${Math.round(desconto)}%</span><small>Desconto: ${formatCurrency(economia)} · Original: ${formatCurrency(totalBruto)}</small>` : `<small>Sem desconto aplicado.</small>`;
    }
  ...
  const generateSobrasMessage = (weekStart = getWeekStart()) => {
    ...
    const weekLabel = formatWeekRangeLabel(weekStart).replace("Semana: ", "");

    if (available.length === 0) return `Não temos pizzas disponíveis para a semana de ${weekLabel}.`;

    const lines = available.map((p) => `• ${p.nome} (${p.tamanho}) — ${p.sobraProj} un.`);
    return `Pizzas disponíveis para retirada (${weekLabel}):\n\n${lines.join("\n")}\n\nSe quiser alguma, me chama que já deixo reservada para você.`;
  };

  const renderStockAvailabilityPanel = ({
    selectId,
    searchId,
    cardsId,
    messageId,
    ...
    const selectSemana = document.getElementById(selectId);
    ...
    const refresh = () => {
      ...
      const tbodyM = massasTableId ? document.querySelector(`#${massasTableId} tbody`) : null;
      if (tbodyM) {
      ...
      const cardsBox = document.getElementById(cardsId);
      if (cardsBox) {
      ...
      const tbodyP = pizzasTableId ? document.querySelector(`#${pizzasTableId} tbody`) : null;
      if (tbodyP) {
  ...
  const renderConsultaRapidaSobras = () => {
    renderStockAvailabilityPanel({
      selectId: "sobras-semana-select",
      searchId: "sobras-search-pizza",
      cardsId: "sobras-cards",
      messageId: "sobras-mensagem",
      massasTableId: "tabela-sobras-massas",
      pizzasTableId: "tabela-sobras-pizzas",
      cardLimit: 12,
    });
    renderStockAvailabilityPanel({
      selectId: "home-sobras-semana-select",
      searchId: "home-sobras-search-pizza",
      cardsId: "home-sobras-cards",
      messageId: "home-sobras-mensagem",
      cardLimit: 8,
    });
  };

  const copyTextToClipboard = async (text, fallbackElement = null) => {
    try {
      await navigator.clipboard.writeText(text);
      showSaveStatus("Mensagem copiada!");
    } catch {
      if (!fallbackElement) {
        showSaveStatus("Não foi possível copiar automaticamente.", false);
        return;
      }
  ...
  const copyStockMessage = async (textareaId) => {
    const textarea = document.getElementById(textareaId);
    if (!textarea) return;
    await copyTextToClipboard(textarea.value, textarea);
  };
  window.copySobrasMensagem = () => copyStockMessage("sobras-mensagem");
  document.getElementById("home-copy-sobras")?.addEventListener("click", () => copyStockMessage("home-sobras-mensagem"));
  ...
  const getFilteredPedidos = (filterRange = "all") => {
    ...
    return database.pedidos.filter((p) => {
      if (!isPedidoAtivoFinanceiro(p)) return false;
      const pedidoDate = getPedidoDataEntrega(p);
      if (!pedidoDate) return filterRange === "all";
      const pDate = parseSafeDate(pedidoDate);
      switch (filterRange) {
        case "today": return pDate >= today && pDate < tomorrow;
        case "week": return pDate >= weekStart && pDate < weekEnd;
        case "month": return pDate >= monthStart && pDate < monthEnd;
        case "all":
        default: return true;
      }
    });
  ...
  const renderDashboard = (filterRange = "all") => {
    ...
    renderNovosClientesChart(filteredPedidos);
    renderPizzaRanking(filterRange);
    renderDashboardCompareCards(filteredPedidos, filterRange);
  };

  const renderBalancoChart = (t) => {
    const e = t.reduce((acc, p) => acc + Number(p.valorFinal || p.valorTotal || 0), 0);
    const a = t.reduce((acc, p) => {
      return acc + (p.items || []).reduce((inner, item) => inner + (item.isCustom ? 0 : calculatePizzaCost(item.pizzaId) * Number(item.qtd || 0)), 0);
    }, 0);
    const r = e - a;
    ...
    chartInstances.balanco = new Chart(canvas.getContext("2d"), {
      ...
      data: {
        labels: ["Balanço"],
        datasets: [
          { label: "Valor dos pedidos", data: [e], backgroundColor: "#2ecc71" },
          { label: "Custo estimado", data: [a], backgroundColor: "#e74c3c" },
          { label: "Margem estimada", data: [r], backgroundColor: "#3498db" }
        ]
      },
  ...
  const renderPizzasMaisLucrativasChart = (t) => {
    const e = t.flatMap((pedido) => pedido.items || []).reduce((t, e) => {
  ...
  const renderVendasPorVendedorChart = (t) => {
    const e = t.reduce((t, e) => {
      if (e.vendedor) t[e.vendedor] = (t[e.vendedor] || 0) + Number(e.valorFinal || e.valorTotal || 0);
      return t;
    }, {});
    ...
    chartInstances.vendedor = new Chart(o, {
      ...
      data: {
        labels: a,
        datasets: [{ label: "Valor dos pedidos", data: r, backgroundColor: "#487eb0" }],
      },
      options: {
        responsive: true,
        maintainAspectRatio: false,
  ...
  const renderPedidosSemanaChart = (pedidos) => {
    ...
    pedidos.forEach((p) => {
      const dateValue = getPedidoDataEntrega(p);
      if (!dateValue) return;
      const ws = getWeekStart(dateValue);
      if (Object.prototype.hasOwnProperty.call(weeksMap, ws)) {
    ...
    chartInstances.semana = new Chart(ctx, {
      ...
      data: {
        labels,
        datasets: [{
          label: "Pizzas nos pedidos (semana)",
          data,
          tension: 0.3,
          fill: false,
  ...
  const renderVendasPorDiaChart = (pedidos) => {
    ...
    const vendasPorDia = pedidos.reduce((acc, pedido) => {
      const dateValue = getPedidoDataEntrega(pedido);
      if (!dateValue) return acc;
      const dia = parseSafeDate(dateValue).toLocaleDateString("pt-BR");
      acc[dia] = (acc[dia] || 0) + Number(pedido.valorFinal || pedido.valorTotal || 0);
      return acc;
    }, {});
    ...
    chartInstances.dia = new Chart(ctx, {
      ...
      data: {
        labels: labels,
        datasets: [{
          label: "Valor dos pedidos por dia",
          data: data,
          borderColor: "#2c3e50",
          tension: 0.1,
          fill: false,
  ...
  const renderPizzaRanking = (filterRange = "all") => {
    ...
    const getOrdersBetween = (start, end) => {
      return database.pedidos.filter(p => {
        const status = normalizePedidoStatusValue(p.status);
        if (status !== "Concluído" && status !== "Pronto") return false;
        const dateValue = getPedidoDataEntrega(p);
    ...
    const getOrdersForWeek = (weekStartString) => {
      const start = parseSafeDate(weekStartString);
      const end = new Date(start);
    ...
    if (filterRange === "custom-week") {
      ...
      if (customWeekStr) {
        ...
        textoComparacao = "semana selecionada e semana anterior";
      }
      ...
      pedidosAtuais = getOrdersBetween(inicioSemana, inicioProximaSemana);
      pedidosAnteriores = getOrdersBetween(inicioSemanaPassada, inicioSemana);
      textoComparacao = "esta semana e semana passada";
      ...
      pedidosAtuais = getOrdersBetween(inicioMes, inicioProximoMes);
      pedidosAnteriores = getOrdersBetween(inicioMesPassado, inicioMes);
      textoComparacao = "este mês e mês passado";
      ...
      pedidosAtuais = getOrdersBetween(hoje, amanha);
      pedidosAnteriores = getOrdersBetween(ontem, hoje);
      textoComparacao = "hoje e ontem";

    } else {
      pedidosAtuais = getOrdersBetween(null, null);
      pedidosAnteriores = [];
      textoComparacao = "histórico completo";
    }
    ...
    top10Atual.forEach((nomePizza, index) => {
      ...
      if (filterRange === "all") {
        icon = '<span class="rank-fire">Histórico</span>';
      } else if (posicaoPassada === null) {
        icon = '<span class="rank-new">Entrou no ranking</span>';
        ...
        icon = '<span class="rank-fire">Manteve a posição</span>';
      }

      html += `
                <li>
                    <div class="rank-pos">#${posicaoAtual}</div>
                    <div class="rank-name">${escapeHTML(nomePizza)}</div>
  ...
  const renderNovosClientesChart = (filteredPedidos) => {
    const canvas = document.getElementById("novosClientesChart");
    if (!canvas) return;

    const dataPrimeiroPedido = {};
    const pedidosVisiveis = new Set(filteredPedidos || database.pedidos);

    database.pedidos.forEach(p => {
      const dateValue = getPedidoDataEntrega(p);
      if (!dateValue || normalizePedidoStatusValue(p.status) !== "Concluído") return;
      const d = parseSafeDate(dateValue);
      const clienteId = p.clienteId || p.cliente;
      if (!clienteId) return;

      if (!dataPrimeiroPedido[clienteId] || d < dataPrimeiroPedido[clienteId].date) {
        dataPrimeiroPedido[clienteId] = { date: d, pedido: p };
      }
    });

    const agrupadoPorMes = {};

    Object.values(dataPrimeiroPedido).forEach(({ date: data, pedido }) => {
      if (!pedidosVisiveis.has(pedido)) return;
  ...
  document.getElementById("btn-lista-compras")?.addEventListener("click", () => {
    ...
    let contentHTML = "<p>Nenhum ingrediente está abaixo do estoque mínimo.</p>";
    ...
    if (itemsBaixos.length > 0) {
      ...
      itemsBaixos.forEach((item) => {
        ...
        contentHTML += `<tr><td>${escapeHTML(item.nome || "")}</td><td>${item.qtd.toFixed(3)}</td><td>${item.estoqueMinimo.toFixed(3)}</td><td><b>${comprar}</b></td></tr>`;
      });
...
(Esta ação apenas adicionará ao estoque, sem baixa de ingredientes).`)) {
  ...
  document.getElementById("export-demanda")?.addEventListener("click", () => {
    const weekStart = document.getElementById("filter-demanda-semana")?.value || getWeekStart();
    const sizeFilter = document.getElementById("filter-demanda-tamanho")?.value || "";
  ...
  document.getElementById("pending-alert-close")?.addEventListener("click", hidePendingOrdersAlert);
  document.getElementById("pending-alert-open")?.addEventListener("click", () => {
    hidePendingOrdersAlert();
    abrirPedidosAguardandoConfirmacao();
  });


  // ===== V4: Caixa, produção, impressão, venda rápida e prioridades de clientes =====
  ...
  const renderQuickSale = () => {
    ...
    box.innerHTML = pizzas.map((pizza) => {
      ...
      const disponivel = getPizzaSobraForWeek(pizza.id, weekStart);
      const stockClass = disponivel <= 0 ? "is-empty" : "";
      return `<div class="quick-pizza ${stockClass}">
        <span>${escapeHTML(pizza.nome)} (${escapeHTML(pizza.tamanho || "")})<small>${formatCurrency(price)} · ${formatStockSobra(pizza.id, weekStart)}</small></span>
  ...
  const saveQuickSale = async () => {
    ...
    const quickVendedorInput = document.getElementById("quick-vendedor");
    const vendedor = (quickVendedorInput?.value || getCurrentSeller()?.nome || "").trim();
    if (quickVendedorInput && !quickVendedorInput.value && vendedor) quickVendedorInput.value = vendedor;
    ...
    const pedidoSalvoRecord = getRpcPedidoRecord(pedidoSalvo);
    showSaveStatus(isFutureWeek(dataEntrega)
      ? "Venda agendada para uma semana futura, sem baixar o estoque da semana atual."
      : pedidoSalvoRecord?.estoque_baixado
        ? "Venda rápida registrada e estoque reservado."
        : "Venda rápida confirmada para produção.");
    quickSaleItems = {};
    document.getElementById("quick-pix-box")?.classList.add("hidden");
    await loadDataFromSupabase();
  };
...
const pixCRC16 = (payload) => { let crc=0xFFFF; for(let i=0;i<payload.length;i++){crc^=payload.charCodeAt(i)<<8; for(let j=0;j<8;j++) crc=(crc&0x8000)?((crc<<1)^0x1021):(crc<<1); crc&=0xFFFF;} return crc.toString(16).toUpperCase().padStart(4,"0"); };
  ...
  const renderClientIntelligence = () => {
    ...
    card.innerHTML = `
      <div class="section-title-row">
        <div>
          <h3>Prioridades de clientes</h3>
          <p class="small-muted">Clientes que pedem atenção por recorrência, pagamento pendente, ticket ou tempo sem comprar.</p>
        </div>
      </div>
        ...
        ${rows.map((row) => {
          ...
          return `<div class="smart-client ${primaryClass}">
            <div class="smart-client-head">
              <b>${row.valorPendente > 0 ? '<span class="debt-dot">!</span> ' : ''}${escapeHTML(row.cliente.nome || "Cliente")}</b>
              <span>${escapeHTML(row.tags.slice(0, 2).join(" · "))}</span>
            </div>
  ...
  const renderHome = () => {
    ...
    alertBox.innerHTML = `<div class="home-list">
      ...
      <div class="home-list-item"><b>Prontos para concluir</b><span class="badge-inline ${pendencias.concluir ? 'danger' : 'success'}">${pendencias.concluir}</span><small>${pendencias.prontosParaConcluir.slice(0,4).map((p) => `${escapeHTML(p.cliente || "Cliente")} · ${formatCurrency(p.valorFinal || p.valorTotal || 0)}`).join('<br>') || 'Nenhuma pendência.'}</small></div>
  ...
  const renderDashboardCompareCards = (filteredPedidos, filterRange = "all") => {
    ...
    const getStats = (pedidos) => {
      ...
      return { pedidos: pedidos.length, receita, pizzas, pendencias };
    };

    const getOrdersBetween = (start, end) => database.pedidos.filter((pedido) => {
      if (!isPedidoAtivoFinanceiro(pedido)) return false;
      const dateValue = getPedidoDataEntrega(pedido);
      if (!dateValue) return false;
    ...
    let previousPedidos = null;
    let previousLabel = "Período anterior";
    const today = parseSafeDate(formatDateToYYYYMMDD(new Date()));
    if (filterRange === "today") {
      const start = new Date(today); start.setDate(start.getDate() - 1);
      previousPedidos = getOrdersBetween(start, today);
      previousLabel = "Ontem";
    } else if (filterRange === "week" || filterRange === "custom-week") {
      const selectedWeek = filterRange === "custom-week"
        ? document.getElementById("dash-week-filter")?.value || getWeekStart()
        : getWeekStart();
      const currentStart = parseSafeDate(selectedWeek);
      const previousStart = new Date(currentStart); previousStart.setDate(previousStart.getDate() - 7);
      previousPedidos = getOrdersBetween(previousStart, currentStart);
    ...
    const cur = getStats(filteredPedidos);
    const prev = previousPedidos ? getStats(previousPedidos) : null;
    const diff = (a, b, currency = false) => {
      const delta = a - b;
      const sign = delta > 0 ? "▲" : delta < 0 ? "▼" : "•";
      return `${sign} ${currency ? formatCurrency(Math.abs(delta)) : Math.abs(delta)}`;
    };
    const detail = (currentValue, previousValue, currency = false) => prev
      ? `${previousLabel}: ${currency ? formatCurrency(previousValue) : previousValue} · ${diff(currentValue, previousValue, currency)}`
      : "Todo o histórico ativo";

    box.innerHTML = `
      <div class="kpi-tile"><span>Pedidos no período</span><b>${cur.pedidos}</b><small>${detail(cur.pedidos, prev?.pedidos)}</small></div>
      <div class="kpi-tile"><span>Receita paga</span><b>${formatCurrency(cur.receita)}</b><small>${detail(cur.receita, prev?.receita, true)}</small></div>
      <div class="kpi-tile"><span>Pizzas nos pedidos</span><b>${cur.pizzas}</b><small>${detail(cur.pizzas, prev?.pizzas)}</small></div>
      <div class="kpi-tile"><span>Valor em aberto</span><b>${formatCurrency(cur.pendencias)}</b><small>${detail(cur.pendencias, prev?.pendencias, true)}</small></div>`;
  };
  ...
  const renderMobileInicio = () => {
    ...
    renderMobileShell(`<section class="m-section">
      <div class="m-card"><h2>Visão geral</h2><p class="m-muted">Resumo da semana atual com pedidos, estoque e produção.</p></div>
      <div class="m-kpis">
  ...
  const renderMobilePedido = () => {
    ...
    document.getElementById("m-add-pizza")?.addEventListener("click", () => {
      ...
      const pagamento = document.getElementById("m-pedido-pagamento").value;
      const weekStart = getWeekStart(document.getElementById("m-pedido-semana")?.value || new Date());
      const disponivel = getAvailableStockForPizza(pizzaId, mobileOrderItems, new Map(), weekStart);
      if (qtd > disponivel) {
...
Lançar mesmo assim como encomenda/produção pendente?`);
  ...
  const saveMobilePedido = async (pedidoData) => {
    pedidoData.vendedor = (pedidoData.vendedor || getCurrentSeller()?.nome || "").trim();
    ...
    try {
      ...
      const newPedido = {
        ...
        status: getInitialPedidoStatus(pedidoData.dataEntrega),
        tipo_pedido: getInitialPedidoType(pedidoData.dataEntrega),
        origem_pedido: "gestao",
        pago: false
      };
      ...
      const pedidoMessage = isFutureWeek(newPedido.dataEntrega)
        ? "Pedido agendado para uma semana futura, sem baixar o estoque da semana atual."
        : pedidoSalvoRecord?.estoque_baixado
          ? "Pedido registrado pelo celular e estoque reservado."
          : "Pedido do celular confirmado para produção.";
      showSaveStatus(pedidoMessage + complementoAviso, !complementoAviso);
      await loadDataFromSupabase();
  ...
  const renderMobileVenda = () => {
    ...
    const renderRows = () => {
      ...
      const selectedWeek = document.getElementById("m-venda-semana")?.value || week;
    ...
    document.getElementById("m-venda-semana")?.addEventListener("change", renderRows);
  ...
  // Abre uma aba de desktop dentro do celular. Alem da classe, precisa desfazer
  // os estilos inline que hardFixMobileLayout aplica (app fixo em tela cheia e
  // scroll travado), senao a aba abre atras do app mobile e nao rola.
  const openDesktopTabOnMobile = (tab) => {
    document.body.classList.add("force-desktop-mobile");
    document.getElementById("mobile-app")?.removeAttribute("style");
    document.querySelectorAll(".mobile-bottom-nav").forEach((el) => el.removeAttribute("style"));
  ...
  const renderMobileMais = () => {
    const week = getWeekStart();
    // No celular o app proprio substitui as abas; sem esta entrada o vendedor
  ...
  const hardFixMobileLayout = () => {
    ...
    if (document.body.classList.contains("force-desktop-mobile")) return;
  ...
  // Filtro da tabela do vendedor. O padrão é "comigo" porque a lista completa
  // tem 80+ sabores e o vendedor normalmente carrega meia dúzia: mostrar tudo
  // por padrão transformava a tela num paredão de linhas zeradas.
  let sellerStockFilter = "comigo";
  ...
  const renderSellerInventory = () => {
    ...
    const receiveCount = document.getElementById("seller-stock-receive-count");
    const tbody = document.querySelector("#seller-stock-table tbody");
    const history = document.getElementById("seller-stock-history");
    if (!title || !toggle || !kpis || !suggestions || !tbody || !history) return;
    ...
    if (!seller) {
      ...
      kpis.innerHTML = suggestions.innerHTML = history.innerHTML = "";
      if (receiveCount) receiveCount.textContent = "0";
      tbody.innerHTML = '<tr><td colspan="5" class="empty-row">Nenhum vendedor selecionado.</td></tr>';
      return;
    }
    ...
    copy.innerHTML = active
      ? '<span class="seller-stock-state on">Ativo</span> Pedidos da loja vinculados a você usam primeiro o seu estoque.'
      : '<span class="seller-stock-state off">Inativo</span> Pedidos da loja seguem a lógica normal do estoque principal.';
    ...
    const totalValue = withStock.reduce((sum, item) => sum + Number(item.qtd || 0) * Number(item.precoVenda || 0), 0);
    const sugg = Array.isArray(painel.sugestoes) ? painel.sugestoes : [];
    const toReceive = sugg.reduce((sum, pedido) => sum + (Array.isArray(pedido.items) ? pedido.items.reduce((s, i) => s + Number(i.qtd || 0), 0) : 0), 0);

    kpis.innerHTML = `
      <div class="seller-stock-kpi"><span>Unidades comigo</span><b>${totalUnits}</b></div>
      <div class="seller-stock-kpi"><span>Sabores diferentes</span><b>${withStock.length}</b></div>
      <div class="seller-stock-kpi"><span>Valor em mãos</span><b>${formatCurrency(totalValue)}</b></div>
      <div class="seller-stock-kpi ${toReceive ? "highlight" : ""}"><span>A receber</span><b>${toReceive}</b></div>`;
    ...
    document.querySelectorAll("[data-seller-filter]").forEach((btn) => {
      btn.classList.toggle("is-active", btn.dataset.sellerFilter === sellerStockFilter);
    });
    const byFilter = stock.filter((item) => {
      const qtd = Number(item.qtd || 0);
      if (sellerStockFilter === "comigo") return qtd > 0;
      if (sellerStockFilter === "acabando") return qtd > 0 && qtd <= 2;
      return true;
    });
    const filtered = byFilter.filter((item) => `${item.nome || ""} ${item.tamanho || ""}`.toLowerCase().includes(search));
    const emptyMessage = search
      ? "Nenhuma pizza com esse nome."
      : sellerStockFilter === "comigo"
        ? 'Você ainda não tem pizzas próprias. Confirme um pedido na Entrada rápida acima ou use o filtro <b>Todas</b> para lançar uma entrada manual.'
        : sellerStockFilter === "acabando"
          ? "Nada acabando: tudo que você tem está com 3 ou mais unidades."
          : "Nenhuma pizza cadastrada no cardápio.";
    tbody.innerHTML = filtered.length ? filtered.map((item) => {
      const qtd = Number(item.qtd || 0);
      const nivel = qtd <= 0 ? "zero" : qtd <= 2 ? "baixo" : "ok";
      return `
      <tr class="seller-stock-row level-${nivel}">
        <td data-label="Pizza"><b>${escapeHTML(item.nome || "")}</b></td>
        <td data-label="Tamanho">${escapeHTML(item.tamanho || "Único")}</td>
        <td data-label="Comigo"><span class="seller-qty ${nivel}">${qtd}</span>${qtd > 0 && qtd <= 2 ? '<small class="seller-qty-note">acabando</small>' : ""}</td>
        <td data-label="Valor em mãos">${qtd > 0 ? formatCurrency(qtd * Number(item.precoVenda || 0)) : "—"}</td>
        <td data-label="Ajuste rápido"><div class="seller-stock-actions">
          <button type="button" class="action-btn remove-btn" ${qtd <= 0 ? "disabled" : ""} title="${qtd <= 0 ? "Sem unidades para retirar" : "Retirar 1 (venda em mãos)"}" onclick="window.adjustSellerStock('${escapeAttr(item.pizzaId)}', -1)">−1</button>
          <button type="button" class="action-btn confirm-btn" title="Adicionar 1" onclick="window.adjustSellerStock('${escapeAttr(item.pizzaId)}', 1)">+1</button>
    ...
    }).join("") : `<tr><td colspan="5" class="empty-row">${emptyMessage}</td></tr>`;

    // --- entrada rápida ---
    if (receiveCount) {
      receiveCount.textContent = String(sugg.length);
      receiveCount.classList.toggle("is-zero", sugg.length === 0);
    }
    suggestions.innerHTML = sugg.length ? sugg.map((pedido) => {
      ...
      const un = items.reduce((sum, item) => sum + Number(item.qtd || 0), 0);
      return `<article class="seller-stock-suggestion">
        <div class="seller-stock-suggestion-main">
          <div class="seller-stock-suggestion-head">
            <b>${escapeHTML(pedido.cliente || "Cliente não informado")}</b>
            <span class="status-chip ${normalizeStatusClass(pedido.status)}">${escapeHTML(pedido.status || "")}</span>
            ${pedido.pago ? '<span class="payment-tag paid">Pago</span>' : '<span class="payment-tag unpaid">A receber</span>'}
          </div>
          <small>Entrega ${escapeHTML(formatDateBR(pedido.dataEntrega || ""))} · ${un} un. · ${formatCurrency(pedido.valorFinal || 0)}</small>
          <div class="seller-stock-suggestion-items">${items.map((item) => `<span class="seller-stock-pill">${Number(item.qtd||0)}x ${escapeHTML(item.nome || "")}${item.tamanho ? ` (${escapeHTML(item.tamanho)})` : ""}</span>`).join("")}</div>
        </div>
        <button type="button" class="action-btn confirm-btn seller-stock-receive-btn" onclick="window.receiveSellerOrder('${escapeAttr(pedido.pedidoId)}')">Recebi essas ${un} un.</button>
      </article>`;
    }).join("") : '<div class="empty-state">Nenhum pedido seu em aberto aguardando retirada do estoque principal.</div>';
    ...
    // 8 em vez de 30: o histórico completo empurrava o resto da página para fora da tela.
    history.innerHTML = movements.length ? movements.slice(0,8).map((mv) => {
  ...
  document.querySelectorAll("[data-seller-filter]").forEach((btn) => btn.addEventListener("click", () => {
    sellerStockFilter = btn.dataset.sellerFilter || "comigo";
    renderSellerInventory();
  }));
  ...
  document.getElementById('loja-config-form')?.addEventListener('input', () => {
    const form = document.getElementById('loja-config-form');
    if (form) form.dataset.dirty = 'true';
  });

  document.getElementById('loja-config-reload')?.addEventListener('click', () => {
    const form = document.getElementById('loja-config-form');
    if (form) form.dataset.dirty = 'false';
    renderLojaConfig();
    showSaveStatus('Configurações recarregadas.');
  });

  document.getElementById('loja-config-form')?.addEventListener('submit', async (event) => {
    event.preventDefault();
    const payload = getLojaConfigFormPayload();
    if (!payload.aceita_retirada && !payload.aceita_entrega) {
      showSaveStatus('Ative retirada, entrega ou os dois.', false);
      return;
    }
    if (!payload.pix_ativo && !payload.dinheiro_ativo && !payload.cartao_ativo && !payload.combinar_ativo) {
      showSaveStatus('Ative pelo menos uma forma de pagamento.', false);
      return;
    }
    try {
      showLoader();
      ...
      if (error) throw error;
      database.loja_config = { ...DEFAULT_LOJA_CONFIG, ...payload };
      const form = document.getElementById('loja-config-form');
      if (form) form.dataset.dirty = 'false';
      showSaveStatus('Configurações da loja salvas.');
      renderLojaConfig();
    } catch (error) {
      showSaveStatus(formatSupabaseError(error, 'Não foi possível salvar as configurações'), false);
    } finally {
      hideLoader();
    }
  });
